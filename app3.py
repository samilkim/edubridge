...
if not search_button:
    ...
    if not user_query.strip():
        ...
        with st.spinner('AI 코치가 최적의 강좌를 분석 중입니다...'):
            try:
                # 1. 키워드 확장
                kw_res = model.generate_content(f"'{user_query}' 관련 디지털 기술 키워드 3개만 쉼표로.")
                ...
                def score_func(idx):
                    ...
                    sim_score = cos_sim[idx] * 60 # 유사도 기본 점수
                    
                    # 지역 가산점 로직 수정
                    if row['시군명'] == selected_city:
                        region_score = 30 # 내 지역 강좌 우선
                        if row['구'] == selected_district:
                            region_score += 10 # 내 구 강좌는 최우선
                    else:
                        region_score = 0 # 타 지역 강좌는 가산점 없음 (하지만 배제하지 않음)
                    
                    vul_score = (vul_row['최종_취약지수'] / 100) * 10
                    return sim_score + region_score + vul_score

                df_lectures['total_score'] = [score_func(i) for i in range(len(df_lectures))]
                # 상위 3개 추출 (내 지역이든 아니든 점수가 높은 순)
                top_results = df_lectures[df_lectures['total_score'] > 0].sort_values('total_score', ascending=False).head(3)
                
                if len(top_results) == 0:
                    st.info("📍 관련 강좌를 찾지 못했습니다. 다른 키워드로 검색해 보세요.")
                else:
                    st.markdown(f'<h3 style="color: #1E3A8A; margin-bottom: 25px;">🎯 "{ai_keywords}" 기반 추천 결과</h3>', unsafe_allow_html=True)
                    
                    # API 호출 최소화: 모든 사유 한 번에 생성
                    lec_list_str = "\n".join([f"- {name}" for name in top_results['강좌명']])
                    combined_prompt = f"사용자 관심: {user_query}. 다음 강좌들에 대해 각각 짧은 추천 이유를 한 문장씩 작성해줘. 형식은 '강좌명: 이유' 줄바꿈 필수.\n{lec_list_str}"
                    ...
                    for i, (idx, row) in enumerate(top_results.iterrows()):
                        ...
                        display_reason = "회원님께 추천하는 디지털 역량 교육입니다."
                        ...
                        with cols[i]:
                            st.markdown(f"""
                                <div class="lecture-card">
                                    <span style="background:#DBEAFE; color:#1E40AF; padding:5px 12px; border-radius:20px; font-size:0.8rem; font-weight:bold;">추천 {i+1} {badge_html}</span>
                                    <div style="font-size:1.15rem; font-weight:800; margin:15px 0; color:#0F172A; min-height:3.2em; line-height:1.4;">{row['강좌명']}</div>
                                    <p style="font-size:0.95rem; color:#475569; line-height:1.6; margin-bottom:15px;">💡 {display_reason}</p>
